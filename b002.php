...
function fGetPost($sHost,$iPostNumber,$c,$sUserKey,$sBlogid,$aPostCategory,$oFileOut)
{
	global $bImageResize,$iResizeWidth,$sNick,$bImageDownload;
...
//본문 처리
	...
	while ($buffer) {
		...
		if (preg_match('/Edit\.content/',$buffer,$aMatch)) {
			$sContent=adjustUTF8(iconv('cp949','utf-8',substr($buffer,17,-4)));
		}
		if (preg_match('/Edit\.mcontent/',$buffer,$aMatch)) {
			$mcontent=adjustUTF8(iconv('cp949','utf-8',substr($buffer,18,-4)));
		}
		...
		if (preg_match('/subject\\"/',$buffer,$aMatch)) {
			preg_match('/VALUE=\\"([^\\"]*)\\"/',$buffer,$aMatch);
			$sSubject=adjustUTF8(iconv('cp949','utf-8',$aMatch[1]));
		}

		if (preg_match('/NAME=moresubject/',$buffer,$aMatch)) {
			preg_match('/VALUE=\\"([^\\"]*)\\"/',$buffer,$aMatch);
			$sMoreSubject=adjustUTF8(iconv('cp949','utf-8',$aMatch[1]));
		}
...
$rt="<post>";
$rt.="<id>".$c."</id>";
$rt.="<visibility>".($bOpen?"public":"private")."</visibility>";
$rt.="<title>".$sSubject."</title>";
$rt.="<content>".$sContent."</content>";
...
function fMakeCommentSql($sHost,$i,$postcount,$sUserKey)
	...
	while (!feof ($fd)) {
		...
		switch(substr($c,0,2))
			...
			$name=adjustUTF8(iconv("cp949","utf-8",$aMatch[1]));
			$secret=(preg_match('/security3.gif\\\"/',$c)?1:0);
			preg_match('/href=\\\\"([^"]*)\\\\" tit/',$c,$aMatch);
			$url=adjustUTF8(iconv("cp949","utf-8",$aMatch[1]));
			$time=strtotime(substr($c,strpos($c,"> at ")+4,17));
			break;
			case '</':
...
$sTemp.="<homepage>".$url."</homepage>";
$sTemp.="<ip>127.0.0.1</ip>";
$sTemp.="</commenter>";
$sTemp.="<content>".$text."</content>";
$sTemp.="<password>49bf6be2050bf1d6</password>";
$sTemp.="<secret>".$secret."</secret>";
$sTemp.="<written>".$time."</written>";
$sTemp.="</comment>";
...
function fMakeTrackbackSql($sHost,$i,$postcount,$sUserKey)
	...
	while($buffer&&!feof($fd))
		...
		$sTitle=$aMatch[1];
			...
			$sBlogName=adjustUTF8(iconv("cp949","utf-8",$sBlogName));
...
$sTemp="<trackback>";
$sTemp.="<url>".$sUrl."</url>";
$sTemp.="<site>".$sBlogName."</site>";
$sTemp.="<title>".$sTitle."</title>";
$sTemp.="<excerpt>".$sContent."</excerpt>";
$sTemp.="<ip>127.0.0.1</ip>";
$sTemp.="<received>".$iTime."</received>";
$sTemp.="</trackback>";
		$rt[]=$sTemp;
			$count++;
		}
...
function init(){
	...
	for($i=0;$i<$iTotalPost;$i++)
	{
		...
		fwrite($oConfigFile,"\$aPostCategory[".$aPost[$i]."]=\"".$aCategory[$aPostCategory[$aPost[$i]]]."\";\n");
	}
   ...
   fwrite($oConfigFile,"\$bImageDownload=".($bImageDownload?$bImageDownload:0).";\n");
   fwrite($oConfigFile,"\$bImageResize=".($bImageResize?$bImageResize:0).";\n");
...
<script type="text/javascript">
            ...
            var rw = window.document.getElementById("resizewidth").value;
				...
				var url = "b002.php?action=init&" + "&egloospass=" + egloospass + "&egloosid=" + egloosid + "&id=" + id + "&ir=" + ir+ "&rw=" + rw;
