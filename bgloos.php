<?php
//bgloos001 alpha version 0.3
// by gofeel@gmail.com
// http://tttt.egloos.com
...
$id="";
$pass="";
$sBlogid=""; // a1234567 또는 b1234567 의 형식 글쓰기의 url을 참고하세요.
$sNick="Testing";  //사용할 닉네임
$bImageResize=True; //이미지의 가로폭 강제 Resize여부
$bImageDownload=True; //이미지 다운로드 Resize여부
$iResizeWidth=500;  //Resize할 범위
...
function fGetPost($sHost,$iPostNumber,$c,$sUserKey,$sBlogid,$aPostCategory)
{
   ...
   //본문 처리
	...
	for($i=0;$i<count($aMatch[0]);$i++) {
		...
		{
		$sContent=str_replace($tp[0],"<img src=\"http://". ( $tp[6] == "#]" ? "pds":substr($tp[6],1,-2 ) ) . ".egloos.com/pds/1/".$tp[2].$tp[1]."\" align=\"".$tp[3]."\" width=\"".$tp[4]."\" height=\"".$tp[5]."\">",$sContent);
		}
	}
...
function init(){
	global $id,$pass,$sBlogid,$sNick,$iMaxPost;
	set_time_limit(0);
	setlocale(LC_TIME, "C");
	header("Content-Type: text/xml;charset=ISO-8859-1");
	header("Cache-Control: no-store, no-cache, must-revalidate");
   ...
   if(!$xxx) {return 1;}
	...
	$aTempPage=auth(array("userid" => $id,"userpwd"=>$pass,"userip"=>""));
...
<script type="text/javascript">
			...
			function pollTaskmaster() {
				var url = "bgloos.php?postid=" + messageHash;
				initRequest(url);
			...
			function submitTask() {
				var url = "bgloos.php?action=init";
...
<hr/>
<p>
이글루-&gt;태터툴즈<br>알파 0.3 (쓸데없이 버전만 늘어가는고나) 
</p>
<!-- 폼의 시작 -->
<form>
